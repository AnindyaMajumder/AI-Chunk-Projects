...
# --- Chat Loop ---
def run_chat_loop():  # <--- Wrap the terminal loop in a function
    chat = ChatSession()
    while True:
        ...
        if user_input.lower() == "exit":
            break
        elif user_input.lower() == "insurance":
            chat.display_insurance_chat()
            continue
        elif user_input.lower() == "flag":
            try:
                idx = int(input("Enter message number to flag: ")) - 1
                flag_type = input("Flag as 'Received from Adjuster' or 'Sent to Adjuster': ")
                msg_id = chat.internal_chat[idx].id
                chat.flag_message(msg_id, flag_type)
        ...
        ai_content = generate_ai_response_with_gpt(user_input, chat.internal_chat)
        chat.add_ai_response(ai_content)
