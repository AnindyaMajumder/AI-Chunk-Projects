...
def generate_response(user_message, text_chunks, embeddings, label_embeddings, prev_queries, mode="coach"):
    ...
    prompt = (
        ...
        f"Reply in the following format for {mode_label} mode:\n"
        "If Coach mode, use this format:\n"
        ...
        "If Friend mode, use this format:\n"
        ...
        "Make sure all parts are empathetic, specific, actionable, and conversational.\n"
        "If appropriate, you may also add a gentle, relevant follow-up question at the end to encourage further conversation."
    )
    response = openai.ChatCompletion.create(
        model="gpt-4-turbo",
        messages=[
